...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.gstackplusplus/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.gstackplusplus/browse-console.log`
- Network: `.gstackplusplus/browse-network.log`
- Dialog: `.gstackplusplus/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.gstackplusplus/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.gstackplusplus/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.gstackplusplus/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.gstackplusplus/browse.json` | Path to state file (CLI passes to server) |
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.gstackplusplus/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `~/.claude/skills/gstackplusplus/`. After making changes:

1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/gstackplusplus && git pull`
3. Rebuild: `cd ~/.claude/skills/gstackplusplus && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/gstackplusplus/browse/dist/browse`
