...
- **YAGNI · KISS · DRY · SOLID enforced across every skill and every platform.** All four principles are now defined once in `scripts/gen-skill-docs.ts` (`generateDesignPrinciples()`) and injected via `{{DESIGN_PRINCIPLES}}` into every skill that does code analysis, review, or design: `/plan-eng-review`, `/review`, `/design-review`, `/plan-design-review`, `/qa`, `/ship`, and all six platform meta-skills. One definition, consistent everywhere.
- **Six platform meta-skills.** Run once per project to configure gstack++ for your AI agent: `/claude` (reference mode, extended thinking, Conductor support), `/codex` (autonomous mode, decision log, hard caps), `/qwen` (chain-of-thought reasoning, `<think>` blocks, phase summaries), `/antigravity` (diff-centric, streaming phase markers, ephemeral workspace), `/cursor` (`.cursorrules`, checkpoint-scoped diffs, `@file:line` findings), `/copilot` (`copilot-instructions.md`, `@workspace` format, Copilot Edits blocks).
- **Principles in the fix loop.** `/qa` Phase 8 (Fix Loop) now explicitly applies YAGNI (fix only what is broken) and KISS (prefer the simplest correct fix) to every change made during automated repair.
- **Principles in the triage phase.** `/plan-eng-review` code quality review now flags YAGNI, KISS, DRY, and SOLID violations as separate line items rather than bundling them all under "DRY".
- **Principle priority order documented.** YAGNI > KISS > DRY > SOLID. When they conflict, prefer fewer abstractions over more. README and all skill references are consistent.

### Changed

- `/plan-eng-review` engineering preferences updated: DRY bullet replaced with a single line referencing the full YAGNI/KISS/DRY/SOLID table, with explicit priority order.
- Recommendation framing updated: "Map reasoning to a specific preference (DRY, explicit > clever...)" now includes the full set: "YAGNI, KISS, DRY, SOLID, explicit > clever, minimal diff".
- README new section "Design principles: YAGNI · KISS · DRY · SOLID" with principle table and explanation of the single-source-of-truth approach.

### For contributors

- New `generateDesignPrinciples()` resolver in `scripts/gen-skill-docs.ts`. Templates use `{{DESIGN_PRINCIPLES}}` — do not inline the table in individual templates.
- New directories: `cursor/`, `copilot/` (joining `claude/`, `codex/`, `qwen/`, `antigravity/` from 0.6.5.0).

## [0.6.5.0] - 2026-03-18

### Added

- **C++ migration complete.** Every skill template rewritten for C++ application, server, and embedded development. cmake, ctest, clang-tidy, cppcheck, AddressSanitizer, UBSan, ThreadSanitizer, valgrind. No more browser/npm/web references in the core workflow skills.
- **Four platform meta-skills: `/claude`, `/codex`, `/qwen`, `/antigravity`.** Configure gstack++ once per project for your AI agent. Settings written to `~/.gstack/config.yaml` as `model_mode: <platform>`.
...
- **`gstackplusplus-diff-scope` categorizes what changed in your branch.** Run `eval $(gstackplusplus-diff-scope main)` and get `SCOPE_FRONTEND=true/false`, `SCOPE_BACKEND`, `SCOPE_PROMPTS`, `SCOPE_TESTS`, `SCOPE_DOCS`, `SCOPE_CONFIG`. Design review uses it to skip silently on backend-only PRs. Ship pre-flight uses it to recommend design review when frontend files are touched.
...
- **`/gstackplusplus-upgrade` now catches stale vendored copies automatically.** If your global gstack++ is up to date but the vendored copy in your project is behind, `/gstackplusplus-upgrade` detects the mismatch and syncs it. No more manually asking "did we vendor it?" — it just tells you and offers to update.
...
- Standalone usage section in `gstackplusplus-upgrade/SKILL.md.tmpl` now references Steps 2 and 4.5 (DRY) instead of duplicating detection/sync bash blocks. Added one new version-comparison bash block.
...
- **Your CEO review visions are saved, not lost.** Expansion ideas, cherry-pick decisions, and 10x visions are now persisted to `~/.gstackplusplus/projects/{repo}/ceo-plans/` as structured design documents. Stale plans get archived automatically. If a vision is exceptional, you can promote it to `docs/designs/` in your repo for the team.

- **Smarter ship gates.** `/ship` no longer nags you about CEO and Design reviews when they're not relevant. Eng Review is the only required gate (and you can disable even that with `gstackplusplus-config set skip_eng_review true`). CEO Review is recommended for big product changes; Design Review for UI work. The dashboard still shows all three — it just won't block you for the optional ones.
...
- **One less thing to copy-paste.** The SLUG computation (that opaque sed pipeline for computing `owner-repo` from git remote) is now a shared `bin/gstackplusplus-slug` helper. All 14 inline copies across templates replaced with `eval $(gstackplusplus-slug)`. If the format ever changes, fix it once.
- **Screenshots are now visible during QA and browse sessions.** When gstack++ takes screenshots, they now show up as clickable image elements in your output — no more invisible `/tmp/browse-screenshot.png` paths you can't see. Works in `/qa`, `/qa-only`, `/plan-design-review`, `/qa-design-review`, `/browse`, and `/gstackplusplus`.
...
- Added `bin/gstackplusplus-slug` helper (5-line bash) with unit tests. Outputs `SLUG=` and `BRANCH=` lines, sanitizes `/` to `-`.
...
- Added `~/.gstackplusplus-dev/plans/` as a local plans directory for long-range vision docs (not checked in). CLAUDE.md and TODOS.md updated.
...
- **`/gstackplusplus-upgrade` always checks for real.** Running `/gstackplusplus-upgrade` directly now bypasses the cache and does a fresh check against GitHub. No more "you're already on the latest" when you're not.

### For contributors

- Split `last-update-check` cache TTL: 60 min for `UP_TO_DATE`, 720 min for `UPGRADE_AVAILABLE`.
- Added `--force` flag to `bin/gstackplusplus-update-check` (deletes cache file before checking).
...
- gstackplusplus-upgrade template: explicit cross-step prose for variable references between bash blocks.

## 0.4.1 — 2026-03-16

- **gstack++ now notices when it screws up.** Turn on contributor mode (`gstackplusplus-config set gstackplusplus_contributor true`) and gstack++ automatically writes up what went wrong — what you were doing, what broke, repro steps. Next time something annoys you, the bug report is already written. Fork gstack++ and fix it yourself.
...
### Added
- **`bin/gstackplusplus-config` CLI** — simple get/set/list interface for `~/.gstackplusplus/config.yaml`. Used by update-check and upgrade skill for persistent settings (auto_upgrade, update_check).
...
- **Auto-upgrade mode** — set `auto_upgrade: true` in config or `GSTACKPLUSPLUS_AUTO_UPGRADE=1` env var to skip the upgrade prompt and update automatically.
- **4-option upgrade prompt** — "Yes, upgrade now", "Always keep me up to date", "Not now" (snooze), "Never ask again" (disable).
- **Vendored copy sync** — `/gstackplusplus-upgrade` now detects and updates local vendored copies in the current project after upgrading the primary install.
- 25 new tests: 11 for gstackplusplus-config CLI, 14 for snooze/config paths in update-check.

### Changed
- README upgrade/troubleshooting sections simplified to reference `/gstackplusplus-upgrade` instead of long paste commands.
...
### Fixed
- **`.gitignore` append failures silently swallowed** — `ensureStateDir()` bare `catch {}` replaced with ENOENT-only silence; non-ENOENT errors (EACCES, ENOSPC) logged to `.gstackplusplus/browse-server.log`.
...
### Added
- **E2E observability** — heartbeat file (`~/.gstackplusplus-dev/e2e-live.json`), per-run log directory (`~/.gstackplusplus-dev/e2e-runs/{runId}/`), progress.log, per-test NDJSON transcripts, persistent failure transcripts. All I/O non-fatal.
...
- **Eval persistence** — results saved to `~/.gstackplusplus-dev/evals/` with auto-comparison against previous run.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.claude/skills/gstackplusplus` fallback from `resolveServerScript()`
- Crash log path references updated from `/tmp/` to `.gstackplusplus/`
...
- **Project-local browse state** — state file, logs, and all server state now live in `.gstackplusplus/` inside the project root (detected via `git rev-parse --show-toplevel`). No more `/tmp` state files.
...
### Changed
- State file location: `.gstackplusplus/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.gstackplusplus/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/gstackplusplus/browse/src/server.ts`
