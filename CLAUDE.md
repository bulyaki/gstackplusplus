...
bun run eval:list    # list all eval runs from ~/.gstackplusplus-dev/evals/
...
to `~/.gstackplusplus-dev/evals/` with auto-comparison against the previous run.
...
When developing gstack++, `.claude/skills/gstackplusplus` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using gstack++ concurrently.

**Check once per session:** Run `ls -la .claude/skills/gstackplusplus` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all gstack++ invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent gstack++ sessions
- During large refactors, remove the symlink (`rm .claude/skills/gstackplusplus`) so the
  global install at `~/.claude/skills/gstackplusplus/` is used instead
...
Contributors can store long-range vision docs and design documents in `~/.gstackplusplus-dev/plans/`.
...
1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/gstackplusplus && git fetch origin && git reset --hard origin/main`
