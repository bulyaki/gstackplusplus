...
gstack++ skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/gstackplusplus/` (your global install). But when you're developing gstack++ itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd gstackplusplus
...
a report to `~/.gstackplusplus/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/gstackplusplus/bin/gstackplusplus-config set gstackplusplus_contributor true
```
...
2. **Check your logs:** `ls ~/.gstackplusplus/contributor-logs/`
3. **Fork and clone gstack++** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where gstack++ annoyed you)
   ln -sfn /path/to/your/gstackplusplus-fork .claude/skills/gstackplusplus
   cd .claude/skills/gstackplusplus && bun install && bun run build
   ```
...
## Working on gstack++ inside the gstackplusplus repo
...
```
gstackplusplus/                    <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── gstackplusplus -> ../../         <- symlink back to repo root
│   ├── review -> gstackplusplus/review
│   ├── ship -> gstackplusplus/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.gstackplusplus-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.gstackplusplus-dev/` for post-mortem debugging and trend analysis.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/gstackplusplus`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop gstack++.** Symlink your gstackplusplus checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/gstackplusplus-checkout .claude/skills/gstackplusplus
cd .claude/skills/gstackplusplus && bun install && bun run build
```
...
```bash
rm .claude/skills/gstackplusplus
```

Claude Code falls back to `~/.claude/skills/gstackplusplus/` automatically.
