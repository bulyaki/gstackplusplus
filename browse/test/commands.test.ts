...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/gstackplusplus-cli-');
    const execPath = path.join(root, '.claude/skills/gstackplusplus/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/gstackplusplus/browse/src/server.ts');
