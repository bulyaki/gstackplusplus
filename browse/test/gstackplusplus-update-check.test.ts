/**
 * Tests for bin/gstackplusplus-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * GSTACKPLUSPLUS_DIR / GSTACKPLUSPLUS_STATE_DIR / GSTACKPLUSPLUS_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'gstackplusplus-update-check');
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      GSTACKPLUSPLUS_DIR: skillDir,
      GSTACKPLUSPLUS_STATE_DIR: stateDir,
      GSTACKPLUSPLUS_REMOTE_URL: `file://${join(skillDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  skillDir = mkdtempSync(join(tmpdir(), 'gstackplusplus-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'gstackplusplus-state-test-'));
  // Link real gstackplusplus-config so update_check config check works
  const binDir = join(skillDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'gstackplusplus-config'), join(binDir, 'gstackplusplus-config'));
});
...
describe('gstackplusplus-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(skillDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(skillDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(skillDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(skillDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(skillDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(skillDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(skillDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(skillDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(skillDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      GSTACKPLUSPLUS_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ GSTACKPLUSPLUS_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
