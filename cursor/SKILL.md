...
## Step 4: Write CURSOR.md

Write the gstack++ entry to `CURSOR.md` (create the file if it does not exist — Cursor reads `CURSOR.md` for project-level instructions alongside `.cursorrules`):
...
After `/document-release` or any skill that updates `CURSOR.md`, also update
`.cursorrules` to keep it in sync. The `.cursorrules` file is Cursor's primary
context source and drifts faster than `CURSOR.md` if not maintained together.
