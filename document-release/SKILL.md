...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.gstackplusplus/*" -not -path "./.context/*" | sort
```
...
```bash
gh pr view --json body -q .body > /tmp/gstackplusplus-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/gstackplusplus-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/gstackplusplus-pr-body-$$.md
```
